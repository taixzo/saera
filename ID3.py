...
def ordorint(c):
	return ord(c) if type(c) in [type(''), type(b'')] else int(c)
...
class ID3:
	...
	def __init__(self, file, name='unknown filename', as_tuple=0):
		...
		try:
			if self.file.read(3) in ('TAG', b'TAG'):
				self.has_tag = 1
				self.had_tag = 1
				...
				if ordorint(self.comment[-2]) == 0 and ordorint(self.comment[-1]) != 0:
					self.track = ordorint(self.comment[-1])
					self.comment = self.comment[:-2]
				else:
					self.track = None

				rawgenre = self.file.read(1)
				self.genre = ordorint(rawgenre)
	...
	def tupleize(self, s):
		if self.as_tuple and type(s) is not type((1,)):
			return (s,)
		else:
			return s
	...
	def legal_genre(self, genre):
		if type(genre) == type(1) and 0 <= genre < len(self.genres):
			return 1
		else:
			return 0
